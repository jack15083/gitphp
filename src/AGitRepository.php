...
class AGitRepository  {
	...
	public function run($command)
	{
		...
		//Yii::warning("proc_open:".$this->gitPath." ".$command);
	 ...
	 * @return string the response from git
	 */
	public function push($remote, $branch = "master", $origin = "master")
	{
		...
		$command = "push ".$remote." ".$branch.":".$origin;
		return $this->run($command);
	}

	public function pull($branch){
		$command = "pull origin ".$branch.":".$branch;
		return $this->run($command);
	}
	...
	public function getRemoteBranches()
	{
		if ($this->_remoteBranches === null) {
			$this->_remoteBranches = array();
			foreach(explode("\n",$this->run("branch -a")) as $branchName) {
				$branchName = trim($branchName, '* ');
				$branch = new AGitBranch($branchName,$this);
				$this->_remoteBranches[$branchName] = $branch;
			}
		}
	 ...
	 */
	public function hasBranch($branch,$isFuzzy = false)
	{
		if ($branch instanceof AGitBranch) {
			$branch = $branch->name;
		}
    ...
    *   获取该分支的提交日志
    */
	public function getBranchLog($name,$line=10){
		if($line > 10 || $line < 1) $line = 10;
		$command = "log ".$name." --pretty=format:\"%h - %an, %ar : %s\" -".$line;
		return explode("\n",$this->run($command));
	}
    ...
    *   获取该分支的最后次提交ID
    */
	public function getLastCommit($branchName){
		$command = "log ".$branchName." --pretty=format:\"%h\" -1";
		return $this->run($command);
	}
    ...
    *   对比两个分支的差异
    */
	public function getDiffFiles($branch1,$branch2){
		$c1 = $this->getLastCommit($branch1);
		$c2 = $this->getLastCommit($branch2);

		$command = "diff ".$c1." ".$c2." --name-only";
		return explode("\n",$this->run($command));
	}
    ...
    *   检查是否有合法的预发分支
    */
	public function checkReleaseBranch($branch){
		$branches = $this->getBranches();
		$outBranch = new AGitBranch("noBranch",$this);
		$remoteBranches = $this->getRemoteBranches();
		foreach($branches as $info){
			if(strpos($info->name,$branch) !== false){
				foreach($remoteBranches as $data){
					if(strpos($data->name,$info->name) !== false){
						$outBranch = $info;
						return $outBranch;
					}
				}
				$this->deleteBranch($info->name);
				return $outBranch;
			}
		}
		return $outBranch;
